      ...
      if (isValidMove(posUser, posUser + posChange)) {
        ...
        if (posUser == game.posExit) {
          if (game.hasPrincess && game.hasGold) {
            game.didWin = true
            status = {message: 'You Win! Welcome back to Earth, ' + game.user}
          } else {
            game.didWin = true
            status = {message: 'Oops, you returned to Earth too soon. You Lose!'}
          }
        }
        if (posUser == game.posPrincess) {
          game.hasPrincess = true
          status = {message: 'You located your ship.'}
        }
        if (posUser == game.posGold) {
          game.hasGold = true
          status = {message: 'You found the space dust.'}
        }
        ...
        if (_.contains(posGhosts, posUser)) {
          if (game.health > 10) {
            ...
            status = {message: 'Ouch! You were struck by an asteroid.', health: game.health}
          } else {
            game.health = 0
            game.didWin = false
            status = {message: 'Ouch! You were struck by an asteroid and were obliterated.', health: game.health}
